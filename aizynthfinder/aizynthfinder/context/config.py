...
from packages.rcr_yield_predictor import RCRYieldPredictorAPI
...
class Configuration:  # pylint: disable=R0902
    ...
    def from_dict(cls, source: StrDict) -> "Configuration":
        ...
        config_obj.use_yield_score = bool(src_copy.get('use_yield_score', False))
        if config_obj.use_yield_score:
            print('Using reaction yield scorer')
            config_obj.yield_scorer = RCRYieldPredictorAPI(use_parrot=src_copy.get('use_parrot', False),cuda_device=-1) 

        else:
            config_obj.yield_scorer = None
        config_obj.yield_score_parameter = float(src_copy.get('yield_score_parameter', 1.2))
