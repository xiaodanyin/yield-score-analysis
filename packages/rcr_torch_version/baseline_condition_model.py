    ...
    def predict_top_combos(self, inputs, return_categories_only=False, c1_rank_thres=2, s1_rank_thres=3, s2_rank_thres=1, r1_rank_thres=3, r2_rank_thres=1):
        ...
        with torch.no_grad():
            ...
            for c1_cdt in c1_cdts:
                ...
                c1_input[0, c1_cdt] = 1
                if c1_input_user == []:
                    c1_sc = c1_pred.softmax(dim=-1).squeeze().numpy()[c1_cdt]
                else:
                    c1_sc = 1
                if s1_input_user == []:
                ...
                for s1_cdt in s1_cdts:
                    ...
                    s1_input[0, s1_cdt] = 1
                    if s1_input_user == []:
                        s1_sc = s1_pred.softmax(dim=-1).squeeze().numpy()[s1_cdt]
                    else:
                        s1_sc = 1
                    if s2_input_user == []:
                    ...
                    for s2_cdt in s2_cdts:
                        ...
                        s2_input[0, s2_cdt] = 1
                        if s2_input_user == []:
                            s2_sc = s2_pred.softmax(dim=-1).squeeze().numpy()[s2_cdt]
                        else:
                            s2_sc = 1
                        if r1_input_user == []:
                        ...
                        for r1_cdt in r1_cdts:
                            ...
                            r1_input[0, r1_cdt] = 1
                            if r1_input_user == []:
                                r1_sc = r1_pred.softmax(dim=-1).squeeze().numpy()[r1_cdt]
                            else:
                                r1_sc = 1
                            if r2_input_user == []:
                            ...
                            for r2_cdt in r2_cdts:
                                ...
                                r2_input[0, r2_cdt] = 1
                                if r2_input_user == []:
                                    r2_sc = r2_pred.softmax(dim=-1).squeeze().numpy()[r2_cdt]
                                else:
                                    r2_sc = 1
...
if __name__ == '__main__':
    ...
    info_path = os.path.join(this_path, 'model')
    weights_path = os.path.join(info_path, 'dict_weights.npy')
    cont.load_nn_model(info_path=info_path, 
